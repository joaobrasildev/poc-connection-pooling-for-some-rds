#!/usr/bin/env python3
"""Regenerate init-databases.sql with proper formatting."""
import os

content = """-- init-databases.sql
-- Initializes tenant databases and test objects on each SQL Server container.

IF NOT EXISTS (SELECT name FROM sys.databases WHERE name = 'tenant_db')
BEGIN
...
sql_path = os.path.join(script_dir, "init-databases.sql")
with open(sql_path, "w") as f:
    f.write(content)
print(f"Written {len(content)} bytes to {sql_path}")
